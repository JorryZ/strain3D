...
class strain3D:
    ...
    def assisCoordCalc(self, minType = 'minDist', division=5., pixelNum=2., lowPart = 'apex', assisCoordCheck=False, vtkName = None, dimlen2 = None):
        ...
        elif minType == 'minDist':
            minDist = self.minDist
            if minDist/division < min(self.dimlen)*pixelNum:
                minValue = minDist/division
                print('minType: minDist - division {0:.2f}, assisCoordCals minValue: {1:.4f}'.format(division,minValue))
            else:
                minValue = min(self.dimlen)*pixelNum
                print('minType: minDist - pixelNum {0:.2f}, assisCoordCals minValue: {1:.4f}'.format(pixelNum,minValue))
            for axes in range(dim):
                if lowPart=='apex':
                    assisCoord[:,axes*dim+axes] = assisCoord[:,axes*dim+axes]- minValue
                elif lowPart=='basal':
                    assisCoord[:,axes*dim+axes] = assisCoord[:,axes*dim+axes]+ minValue
            self.assisCoord=assisCoord.copy()
        
        if assisCoordCheck == True:
            if type(vtkName)==type(None):
                print('Input error: please input a myocardiam vtk for assisCoordCheck')
                sys.exit()
            vtkData=medImgProc.imread(vtkName)
            vtkData.dim=['z','y','x']
            vtkData.dimlen=dimlen2
            vtkData.rearrangeDim(['x','y','z'])
            
            temp = np.ones((len(self.sampleCoord),1))
            for i in range(dim):
                for j in range(len(self.sampleCoord)):
                    if vtkData.getData(assisCoord[j,i*dim:(i+1)*dim])==0:
                        temp[j] = 0
                        
            sampleCoord = np.array([self.sampleCoord[i,:] for i in range(len(self.sampleCoord)) if temp[i]!=0.])
            self.sampleCoord = sampleCoord.copy()
            self.assisCoord = np.array([assisCoord[i,:] for i in range(len(assisCoord)) if temp[i]!=0.])
            if len(self.sampleCoord)!=len(self.longitAxis):
                temp2 = np.array([self.longitAxis[i,:] for i in range(len(self.longitAxis)) if temp[i]!=0.])
                self.longitAxis = temp2.copy()
                temp2 = np.array([self.circumAxis[i,:] for i in range(len(self.circumAxis)) if temp[i]!=0.])
                self.circumAxis = temp2.copy()
                temp2 = np.array([self.radialAxis[i,:] for i in range(len(self.radialAxis)) if temp[i]!=0.])
                self.radialAxis = temp2.copy()
    ...
    def vectorCalc(self):
        ...
        vectorX=coordMotionX-coordMotionA
        vectorY=coordMotionY-coordMotionA
        vectorZ=coordMotionZ-coordMotionA
...
class strain2D:
    ...
    def __init__(self):
        ...
        self.time=None
        self.origin=None
        self.spacing=None   #grid spacing (x,y)
        self.period=None
        self.coefMat=[]
        self.shape=None
        self.coordMat=None
        ...
        self.dimlen=None    #physical length of each (x,y) pixel
        self.sampleCoord =None
        self.sampleNormal=None
        self.bsFourier=BsplineFourier.BsplineFourier()
        ...
        self.eigenVector = None
        self.eigenValue = None
        print('strain2D_init_ done')
        
    def initialize(self,coefFile=None,origin=None,shape=None,spacing=None,period=None,time=None,fourierFormat=None,fileScale=1.,delimiter=' ',getSampleCoord=False,spacingDivision=[1.,1.5],gap=0):   
        if type(coefFile)!=type(None):                                         #initialise bsf file
            self.bsFourierPath = coefFile
            self.bsFourier.readFile(coefFile=coefFile,origin=origin,shape=shape,spacing=spacing,fourierFormat=fourierFormat,delimiter=delimiter)
            self.origin=self.bsFourier.origin
            self.spacing=self.bsFourier.spacing[:2]
            self.period=self.bsFourier.spacing[2]
            self.coefMat=self.bsFourier.coef.copy()
            self.shape=self.coefMat.shape
            self.coordMat = np.mgrid[self.origin[0]:(self.origin[0]+(self.shape[0]-0.1)*self.spacing[0]):self.spacing[0], self.origin[1]:(self.origin[1]+(self.shape[1]-0.1)*self.spacing[1]):self.spacing[1]].reshape(2,*self.shape[:2]).transpose(1,2,0)
            #self.coordMat = np.mgrid[self.origin[0]:(self.origin[0]+(self.shape[0]-0.1)*self.spacing[0]):self.spacing[0], self.origin[1]:(self.origin[1]+(self.shape[1]-0.1)*self.spacing[1]):self.spacing[1],self.origin[2]:(self.origin[2]+(self.shape[2]-0.1)*self.spacing[2]):self.spacing[2]].reshape(3,*self.shape[:3]).transpose(1,2,3,0)
            #self.coefMat=self.bsFourier.coef.reshape(-1, order='F')
        if getSampleCoord==True:
            self.sampleCoord=np.array(self.bsFourier.samplePoints(spacingDivision=spacingDivision[0],gap=gap))  #self.bsFourier.samplePoints()
